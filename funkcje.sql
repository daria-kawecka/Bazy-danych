...
---4---
CREATE FUNCTION CardNUM (@order_num NVARCHAR(25))
RETURNS TABLE 
AS
RETURN
	(SELECT Credit_Card.CardNumber FROM [AdventureWorks2017].[Sales].[SalesOrderHeader] Sales
	INNER JOIN [AdventureWorks2017].[Sales].[CreditCard] Credit_Card ON Sales.CreditCardID = Credit_Card.CreditCardID 
	WHERE @order_num=Sales.SalesOrderNumber)
